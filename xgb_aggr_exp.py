...
def load_data():
    ...
    for col in catCols:
        ...
        aggr = sqldf("select %s , avg(Hazard) as avg_hazard_%s, count(1) as cnt_%s from trainData group by 1" % (col, col,col), locals())
        train_tmp = sqldf("select b.avg_hazard_%s, cnt_%s from trainCat a left join aggr b on a.%s = b.%s " %(col,col,col,col),locals())
        test_tmp = sqldf("select b.avg_hazard_%s, cnt_%s from testCat a left join aggr b on a.%s = b.%s " %(col,col,col,col),locals())
